...
class ModelRunner(object):
  ...
  def evaluate(self,split,return_result):  ### here we add split as a part of argument to take both dev and test dataset
    return {task.name: self.evaluate_task(task,split,return_result) for task in self._tasks}
...
def run_finetuning(config: configure_finetuning.FinetuningConfig):
  ...
  while config.num_trials < 0 or trial <= config.num_trials:
    ...
    if config.do_eval:
      ...
      results.append(model_runner.evaluate('dev',True))
      ...
      if config.write_test_outputs and trial <= config.n_writes_test:
        ...
        for task in tasks:
          ...
          if task.name in ["cola", "mrpc", "mnli", "sst", "rte", "qnli", "qqp",
            ...
            scorer = model_runner.evaluate_task(task)
    ...
    if config.do_test:
      ...
      test_results.append(model_runner.evaluate('test',True))
    if config.evaluate_bioasq: ## loop through all bioasq batches
      for task in tasks:
       for batch in range(1,6):
              scorer = model_runner.evaluate_task(task,"test"+str(batch), False)
              scorer.write_predictions("test"+str(batch))
