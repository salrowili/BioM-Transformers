...
class SpanBasedQAScorer(scorer.Scorer):
  ...
  def write_predictions(self,split_name):
    ...
    if "bioasq" in self._name:
    	utils.write_json(dict(eval(str(all_nbest_json))),self._config.qa_nbest_file(self._name+"-"+split_name))
    	utils.write_json(dict(all_predictions),
                     self._config.qa_preds_file(self._name+"-"+split_name))
    if self._v2:
      if "bioasq" in self._name:
    	  utils.write_json(dict(eval(str(all_nbest_json))),self._config.qa_nbest_file(self._name+"-"+split_name))
    	  utils.write_json(dict(all_predictions),self._config.qa_preds_file(self._name+"-"+split_name))
      else:
        utils.write_json({k: float(v) for k, v in six.iteritems(scores_diff_json)},self._config.qa_na_file(self._name))
        utils.write_json(dict(eval(str(all_nbest_json))),self._config.qa_nbest_file(self._name))
