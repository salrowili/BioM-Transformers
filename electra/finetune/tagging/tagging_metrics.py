...
class EntityLevelF1Scorer(F1Scorer):
  ...
  def _get_results(self):
    print("Evaluation on Named Entity")
    
    self._n_correct, self._n_predicted, self._n_gold = 0, 0, 0
    #with open('predicted.tsv', 'wt') as out_file:
     # tsv_writer = csv.writer(out_file, delimiter='\t')
      #for labels, preds in zip(self._labels, self._preds):
       # for real_label,predicted_label in zip([self._inv_label_mapping[x] for x in labels], [self._inv_label_mapping[x] for x in preds]):
        #  tsv_writer.writerow([real_label, predicted_label])
    for labels, preds in zip(self._labels, self._preds):
