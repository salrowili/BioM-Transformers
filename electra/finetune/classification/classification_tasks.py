...
### here we add chemprot task ##
class ChemProt(ClassificationTask):
  """Chemprot"""

  def __init__(self, config: configure_finetuning.FinetuningConfig, tokenizer):
    super(ChemProt, self).__init__(config, "chemprot", tokenizer, ["CPR:4","CPR:5","CPR:9","CPR:6","CPR:3","false"]) 

  def _create_examples(self, lines, split):
      return self._load_glue(lines, split, 1, None, 2, False) ## 1: position of text, 2: position of label, True: skip the first line

  def get_scorer(self,split):
    return classification_metrics.F1_Chemprot()
