...
def get_task(config: configure_finetuning.FinetuningConfig, task_name,
  ...
  #### BioM-Transformers Tasks ####
  elif task_name == "NCBI-disease":
    return tagging_tasks.NCBI(config, tokenizer)
  elif task_name == "BC5CDR-disease":
    return tagging_tasks.BC5CDRdisease(config, tokenizer)
  elif task_name == "BC5CDR-chem":
    return tagging_tasks.BC5CDRchem(config, tokenizer)
  elif task_name == "bioasq-squadv2":
    return qa_tasks.BioASQv2(config, tokenizer)
  elif task_name == "bioasq-squadv1":
    return qa_tasks.BioASQv1(config, tokenizer)
  else:
